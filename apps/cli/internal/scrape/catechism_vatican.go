...
const vatPromulgatedYr = 1992 // Fidei Depositum

type vatLangCfg struct {
	base          string // page base URL
	suffix        string // per-language filename suffix, e.g. "_sp.html"
...
func NewVaticanCatechismScraper(fetcher Fetcher, lang string) (*VaticanCatechismScraper, error) {
	...
	if !ok {
		return nil, fmt.Errorf("unsupported Vatican catechism language %q (want es or la)", lang)
	}
...
var (
	...
	// A handful of paragraphs lost their bold tag on the source and open with
	// a bare number after the <p> (`<p align="left">168 La Iglesia…`); those
	// are the second alternative and are range-guarded when parsed.
	vatBoldNumRe = regexp.MustCompile(`(?i)<b>\s*(?:<a[^>]*>\s*)?(\d+)\s*(?:</a>)?\s*</b>|<p[^>]*>\s*(\d{1,4})\s`)
...
func (s *VaticanCatechismScraper) stems(ctx context.Context) ([]string, error) {
	...
	stemRe := regexp.MustCompile(`(?i)href="([a-z0-9_-]+)` + regexp.QuoteMeta(s.cfg.suffix))
	var out []string
	seen := map[string]bool{}
	for _, m := range stemRe.FindAllStringSubmatch(string(raw), -1) {
		stem := m[1]
		if stem == "index" || seen[stem] {
			continue
		}
		seen[stem] = true
		out = append(out, stem)
