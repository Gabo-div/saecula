import type en from './en';

// Ecclesiastical Latin — the app's third content language deserves a UI too.
const la: typeof en = {
  common: {
    appName: 'Saecula',
    tagline: 'Viginti saecula, una linea temporum',
    cancel: 'Cancella',
    delete: 'Dele',
  },
  tabs: {
    home: 'Domus',
    calendar: 'Calendarium',
    saints: 'Sancti',
    bible: 'Biblia',
    catechism: 'Catechismus',
    explore: 'Explora',
    profile: 'Persona',
  },
  catechism: {
    title: 'Catechismus',
    loadError: 'Catechismus legi non potuit',
    translation: 'Versio',
  },
  calendar: {
    title: 'Calendarium',
    readings: 'Lectiones diei',
    readingsDesc: 'Lectiones Missae diei',
    saints: 'Sancti',
    saintsDesc: 'Festa et memoriae',
    celebrations: 'Celebrationes',
    celebrationsDesc: 'Annus liturgicus',
    today: 'Hodie',
    loadError: 'Lectiones legi non potuerunt',
    noReadings: 'Nullae lectiones huic diei praesto sunt.',
    calendarLoadError: 'Calendarium legi non potuit',
    seasons: 'Tempora',
    solemnitiesAndFeasts: 'Sollemnitates et festa',
    optional: 'ad libitum',
    holyDay: 'Dies festus de praecepto',
    saintsEmpty: 'Tantum dies feriales hoc mense.',
    celebrationsEmpty: 'Nullae sollemnitates aut festa hoc mense.',
    readingTypes: {
      reading_1: 'Lectio prima',
      reading_i: 'Lectio prima',
      reading_2: 'Lectio secunda',
      reading_ii: 'Lectio secunda',
      responsorial_psalm: 'Psalmus responsorius',
      psalm: 'Psalmus',
      sequence: 'Sequentia',
      gospel: 'Evangelium',
      epistle: 'Epistula',
    },
  },
  home: {
    title: 'Hodie',
    dailyVerse: 'Versiculus diei',
    dailyCatechism: 'Catechismus diei',
    prayers: 'Preces',
    ask: 'Interroga',
    bible: 'Biblia',
    celebrationOfDay: 'Celebratio diei',
  },
  streak: {
    title: 'Continuatio',
    tagline: 'Omnis dies quo oras vel legis continuationem auget.',
    current: 'continuatio praesens',
    best: 'optima continuatio',
    today: 'Hodie perfectum',
    notToday: 'Nondum hodie',
    daysThisYear_one: '{{count}} dies hoc anno',
    daysThisYear_other: '{{count}} dies hoc anno',
    weeks_one: '{{count}} hebdomada continua',
    weeks_other: '{{count}} hebdomadae continuae',
    history: 'Historiam videre',
    completedTitle: 'Dies perfectus!',
    keepGoing: 'Persevera, ne continuationem frangas.',
    continue: 'Perge',
  },
  prayers: {
    individual: 'Orationes',
    guided: 'Orationes ductae',
    language: 'Lingua',
    todaysMysteries: 'Mysteria hodierna',
    mysteries: 'Mysteria',
    order: 'Ordo orationis',
    forEachMystery: 'Pro singulis quinque mysteriis',
    begin: 'Incipe',
    finished: 'Oratio completa',
    again: 'Iterum orare',
  },
  reader: {
    accessibility: 'Accessibilitas',
    fontSize: 'Magnitudo litterarum',
    searchPlaceholder: 'Quaerere…',
    searchHint: 'Scribe saltem duas litteras ut quaeras.',
    noResults: 'Nihil inventum',
  },
  bible: {
    title: 'Biblia',
    bookAndVersion: 'Liber et versio',
    chapter: 'caput',
    version: 'Versio',
    oldTestament: 'Vetus Testamentum',
    newTestament: 'Novum Testamentum',
    chaptersAbbr: 'cap.',
    loadError: 'Caput legi non potuit',
  },
  explore: {
    title: 'Explora',
    comingTitle: 'Mox aderit',
    comingBody: 'Linea temporum mox hic praesto erit.',
  },
  profile: {
    title: 'Persona',
    account: 'Ratio',
    signOut: 'Exire',
  },
  apiKeys: {
    title: 'Claves API',
    subtitle: 'Sine adiutorem artificiosum bibliothecam tuam per MCP legere.',
    newKey: 'Clavis nova',
    namePlaceholder: 'Nomen (ex. gr. portatile)',
    create: 'Creare',
    copyOnce: 'Nunc describe: haec clavis iterum non ostenditur.',
    copy: 'Describere',
    copied: 'Descripta',
    done: 'Factum',
    revokeConfirm: 'Quidquid hac clave utitur statim desinet operari.',
    revoke: 'Revocare',
    empty: 'Nullae adhuc claves.',
    calls: 'vocationes',
    errors: 'errores',
    loadError: 'Claves tuae legi non potuerunt',
    createError: 'Clavis creari non potuit',
    tooMany: 'Nimium claves vigentes. Unam prius revoca.',
    revokeError: 'Clavis revocari non potuit',
  },
  settings: {
    title: 'Optiones',
    theme: 'Aspectus',
    dark: 'Obscurum',
    amoled: 'AMOLED',
    light: 'Clarum',
    accent: 'Color accentus',
    language: 'Lingua',
    translation: 'Versio Bibliae',
    defaultTranslation: 'Praedefinita (secundum linguam)',
  },
  login: {
    email: 'Email',
    password: 'Tessera',
    signIn: 'Ingredi',
    createAccount: 'Rationem crea',
    haveAccount: 'Rationem iam habes? Ingredere',
    noAccount: 'Rationem non habes? Inscribere',
    connectionFailed: 'Conexio defecit',
  },
  chat: {
    title: 'Interroga',
    placeholder: 'Interroga de Scriptura…',
    empty: 'Interroga ut incipias.',
    newChat: 'Novum colloquium',
    history: 'Historia',
    emptyHistory: 'Nulla adhuc colloquia.',
    thinking: 'Cogitans…',
    consulting: 'Consulens {{source}}…',
    error: 'Aliquid erravit. Iterum conare.',
    retry: 'Iterum conare',
    disabled: 'Auxiliator nondum praesto est.',
    deleteConfirm: 'Hoc colloquium delere?',
    toolDone: 'Factum',
    toolWorking: 'Operatur…',
    toolInput: 'Initium',
    toolOutput: 'Exitus',
    copy: 'Exscribe',
    copied: 'Exscriptum',
    details: 'Singula',
    close: 'Claude',
    references: 'Loci',
    model: 'Pondus genit',
    tools: 'Instrumenta',
    noTools: 'Nulla instrumenta pro hac responsione adhibita sunt.',
    toolSearchScripture: 'Scripturas perscrutans…',
    toolGetVerses: 'Locum Scripturae legens…',
    toolSearchCatechism: 'Catechismum quaerens…',
    toolGetCatechism: 'Catechismum legens…',
    toolGraphRelated: 'Loci cognati quaeruntur…',
    toolSearchScripturePast: 'Scripturas perscrutavit',
    toolGetVersesPast: 'Locum Scripturae legit',
    toolSearchCatechismPast: 'Catechismum quaesivit',
    toolGetCatechismPast: 'Catechismum legit',
    toolGraphRelatedPast: 'Loci cognati quaesiti sunt',
    suggestions: 'Quaestiones suggestae',
    suggestWhatDoesChurchTeach: 'Quid Ecclesia de gratia docet?',
    suggestCatechismOnFaith: 'Quid Catechismus de fide dicit?',
    suggestDailyGospel: 'Quomodo Evangelium hodiernum possum vivere?',
    suggestExplainBaptism: 'Explica mihi significationem Baptismi.',
  },
  bookmarks: {
    title: 'Servata',
    all: 'Omnes',
    highlighted: 'Illuminati',
    withNotes: 'Cum notis',
    empty: 'Nondum versiculi servati sunt. Pressionem tene in quolibet versiculo in Biblia ut serves.',
    save: 'Servare versiculum',
    saved: 'Servatum',
    remove: 'Tollere ex servatis',
    highlight: 'Illuminare',
    addNote: 'Addere notam',
    editNote: 'Mutare notam',
    notePlaceholder: 'Scribe notam personalem…',
    saveNote: 'Servare notam',
    share: 'Communica',
    shareImage: 'Partiri ut imaginem',
    background: 'Fundus',
    bgNone: 'Sine fundo',
    bgImage: 'Imago',
    shareText: 'Partiri ut textum',
    copyLink: 'Copiare vinculum',
    shareVia: 'Partitum per Saecula',
    saveError: 'Versiculum servari non potuit',
    removeError: 'Versiculum tolli non potuit',
    highlightError: 'Illuminatio applicari non potuit',
    noteError: 'Nota servari non potuit',
    multiSelect: '{{count}} selecti',
    multiSelectOne: '1 selectus',
    highlightSelected: 'Illuminare selectos',
    noteForSelected: 'Addere notam selectis',
    noteForSelectedPlaceholder: 'Scribe notam pro versiculis selectis…',
    shareMulti: 'Partiri ut textum',
    shareMultiImage: 'Partiri ut imaginem',
    shareMultiImageHint: 'Max. 3 versiculi pro imagine',
    imageSaved: 'Imago in galeria servata',
    imageSaveError: 'Imago servari non potuit',
    note: 'Nota',
    related: 'Relata',
    relatedEmpty: 'Nihil relatum adhuc.',
    dragMore: 'Deduc sursum ut plura videas',
    format: 'Forma',
  },
};

export default la;
